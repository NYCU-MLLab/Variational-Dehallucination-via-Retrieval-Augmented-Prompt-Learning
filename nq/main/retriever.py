...
class Retriever(torch.nn.Module):
    ...
    @torch.inference_mode()
    def retrieve(self, query,train=True,largest=True,baseline=False):
        '''
        return k retrieved id and similarity
        '''
        if type(query[0])==str:
            query = self.model.tokenizer(text=query,return_tensors="pt",padding=True,truncation=True,max_length=512)
