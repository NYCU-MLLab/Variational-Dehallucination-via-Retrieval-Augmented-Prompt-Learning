...
def trainer(epoch):
    ...
    for tokens, masks, targets, querys, answers,_ ,q_a in bar:
        ...
        z.retain_grad()
        #predefine the variance, only minimize the means between two gaussion
        ...
        # beta=kl_anneal_function(i,epoch, train_config["max_epoch"], train_config["start_beta"], train_config["end_beta"],len(train_loader))
        loss+=1*kl
...
def inference(mode,loader):
    ...
    with torch.no_grad():
        for _, _, _, querys, answers,all_as,_ in bar:
            ...
            bar.set_postfix_str(f'EM: {sum(acc)/len(acc):.4f} ')
