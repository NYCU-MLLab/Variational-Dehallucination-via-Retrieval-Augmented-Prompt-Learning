...
class LLaMa:
    def __init__(self, model_dir,hf_access_token=config['hf_access_tokens']):
        ...
        self.chat_history = []
        self.system_prompt = "You are a QA system."#"""You are a helpful, respectful and honest assistant. Always answer as helpfully as possible, while being safe.  Your answers should not include any harmful, unethical, racist, sexist, toxic, dangerous, or illegal content. Please ensure that your responses are socially unbiased and positive in nature.
    ...
    def forward(self, ids, target=None, masks=None, soft_examples=None,output_hidden_states=False):
        ...
        if output_hidden_states: 
            hh=soft_examples.shape[1]
            h=output.hidden_states[16][:,hh,:]

            return output.logits, output.loss,target,h
        return output.logits, output.loss,target

    @torch.inference_mode()
    def generate(self, message, soft_examples=None, max_new_tokens = 1024, streamer=True,return_eigenscore=False,return_hidden_states=False):
