...
class Retriever(torch.nn.Module):
    ...
    @torch.inference_mode()
    def retrieve(self, query,train=True,largest=True,baseline=False,random_select=False):
        '''
        return k retrieved id and similarity
        '''
        if type(query[0])==str:
            query = self.model.tokenizer(text=query,return_tensors="pt",padding=True,truncation=True,max_length=config['max_len'])
        ...
        if train:
            ...
            for batch_index in range(0,id.shape[0]):
                ...
                topk_examples=' '.join(topk_examples)
...
if __name__=='__main__':
    ...
    query='who is the richest guy in the world?'
    print(type(query))
    topk_examples=retriever.retrieve(query)
    print(topk_examples)