...
def trainer(epoch):
    ...
    for tokens, masks, targets, querys, answers,_ ,q_a in bar:
        ...
        topk_examples_prior=retriever.retrieve(querys,random_select=False)# list Bxn
...
def inference(mode,loader,output_files=False):
    ...
    with torch.no_grad():
        for _, _, _, querys, answers,all_as,q_a in bar:
            ...
            topk_examples=retriever.retrieve(querys,train=False,random_select=False)
...
if __name__=='__main__':
    ...
    for i in range(train_config['max_epoch']):
        trainer(i)
        torch.save(prior_PE.state_dict(),f'save/prior_PE_one_{i+1}.pt')
