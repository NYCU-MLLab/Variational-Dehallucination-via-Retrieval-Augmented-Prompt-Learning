...
class QADataset(Dataset):
    def __init__(self,mode='train'):
        self.dataset = load_dataset("lucadiliello/triviaqa")
        self.dataset=self.dataset[mode]
    def __len__(self):
        return len(self.dataset)

    def __getitem__(self, idx):
        question=self.dataset[idx]['question'].replace('？','').replace('?','').strip()
        answer=self.dataset[idx]['answers']
        all_a=answer
        answer = answer[np.random.randint(len(answer))]
        return question,answer,all_a
...
def collect_fn(batch):
    ...
    output=tokenizer(text=q_a_list,return_tensors="pt",padding=True,truncation=True,max_length=128)
    
    return output,q_a_list

class collateLM():
    def __init__(self, max_len=128, tokenizer=None):
